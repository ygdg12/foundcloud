  ...
  const fetchItems = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("LostItems - Raw API response:", data)
        console.log("LostItems - First item raw data:", data.items?.[0])
        
        const normalizedItems = (data.items || []).map((item) => {
          console.log(`LostItems - Raw item ${item._id}:`, item)
          console.log(`LostItems - Raw images for ${item._id}:`, item.images, "Type:", typeof item.images, "Is Array:", Array.isArray(item.images))
          ...
          if (item.images) {
            ...
            if (Array.isArray(item.images)) {
              images = item.images
                .filter(img => img && img !== null && img !== undefined && String(img).trim() !== '')
                .map(img => String(img).trim())
            } else if (typeof item.images === 'string') {
              // Single image as string
              images = [item.images.trim()]
            }
          }
          
          console.log(`LostItems - Processed images for item ${item._id}:`, images)
        ...
        console.log("LostItems - Normalized items with images:", normalizedItems.map(i => ({ id: i._id, title: i.title, images: i.images })))
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                {(() => {
                  ...
                  console.log(`LostItems - Rendering item ${item._id}: hasImages=${hasImages}, images=`, item.images)
                  ...
                  if (imgPath.startsWith("http://") || imgPath.startsWith("https://")) {
                    ...
                    imageSrc = `${BASE_URL}/uploads/lost-items/${imgPath}`
                  }
                  
                  console.log(`LostItems - Image src for item ${item._id}:`, imageSrc, "from path:", imgPath)
                    ...
                    <div className={`${viewMode === "list" ? "w-full sm:w-64 h-48" : "h-48"} relative overflow-hidden`}>
                      <img
                        ...
                        onError={(e) => {
                          ...
                          console.error(`LostItems - Failed to load image for item ${item._id}:`, {
                          ...
                          const altPaths = [
                            ...
                            `${BASE_URL}/uploads/lost-items/${imgPath.split('/').pop()}`,
                            imgPath
                          ]
                          ...
                          if (nextIndex < altPaths.length) {
                            console.log(`LostItems - Trying alternative path ${nextIndex + 1}:`, altPaths[nextIndex])
                        ...
                        onLoad={() => {
                          console.log(`✅ LostItems - Successfully loaded image for item ${item._id}:`, imageSrc)
                        }}
                      />
